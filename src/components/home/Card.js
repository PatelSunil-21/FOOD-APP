    ...
    <div className="box">
      <div className="w-80 rounded-lg bg-white overflow-hidden dark:bg-black border-gradient">
        ...
        <div className="flex px-4 justify-between">
          <select
            className="   p-1 text-black hover:font-bold font-semibold cursor-pointer dark:text-gray-300  border border-black dark:border-gray-400 rounded"
            onChange={handleQty}
          >
          ...
          >
            {priceOptions.map((options,index) => {
              return (
                <option className=" text-black" value={options} key={index}>
                  {options}
                </option>
              );
            })}
